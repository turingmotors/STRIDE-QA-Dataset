...
def draw_mask_and_number_on_image(
    image_rgb, masks, labels, label_mode="1", alpha=0.1, anno_mode=["Mask"]
):
...
def draw_som_on_image(
    detections_dict, image_rgb, label_mode="1", alpha=0.1, anno_mode=["Mask"]
):
    # img_width, img_height = image_rgb.size
    visual = Visualizer(image_rgb, metadata=metadata)
    label = 0
    ...
    for i in range(len(detections_dict["xyxy"])):
        detection = {key: detections_dict[key][i] for key in detections_dict}
        detection["caption"] = tags[i]
        detections_list.append(detection)

    return im, detections_list
...
class Visualizer:
    ...
    def draw_binary_mask_with_number(
        ...
        *,
        ...
        """
        if color is None:
            # Set all colors to green for consistent visualization
            color = (0, 1, 0)  # Green color in RGB
        color = mplc.to_rgb(color)
